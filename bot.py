...
async def check_spam(user_id):
    ...
    # Исправлена ошибка расчёта секунд для предотвращения сбоев при длительных сессиях
...
async def admin_panel(message: types.Message):
    ...
    if len(args) > 1 and args[1] == "users":
        ...
        await message.answer(text, parse_note="Markdown")
        return
...
async def handle_messages(message: types.Message):
    ...
    # Безопасная отправка ответа ИИ без строгого Markdown во избежание сбоев парсинга
