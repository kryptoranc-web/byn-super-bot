...
async def update_data():
    ...
    while True:
        try:
            ...
            logging.info("✅ Данные успешно обновлены в фоне для всех городов")
...
async def ai_forecast_handler(message: types.Message):
    ...
    await message.channel_action if hasattr(message, 'channel_action') else None
...
async def confirm_payment(message: types.Message):
    ...
    if db.add_pending_payment(message.from_user.id, code, "29.90 BYN"):
        # Уведомляем админа о новом платеже через заложенную логику
        try:
